...
class Program(My_Diary_window.Tk, My_Diary_interface.My_Diary_interface):
	...
	def __open_ai_window__(self: typing.Self) -> None:
		if platform.system() == f"Linux":
			os.popen(f"python3 Apps\My_Diary\My_Diary_AI_window.py")

		else:
			os.startfile(f"Apps\My_Diary\My_Diary_AI_window.py", show_cmd=False)
	...
	def __exit__(self: typing.Self) -> None:
		if locale.getdefaultlocale()[0] == f"sr_RS":
			...
			if self.main_screen_exit: self.destroy()
